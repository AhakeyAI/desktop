...
final class AgentManager: ObservableObject {
    ...
    /// 同步执行，需在后台线程调用。nonisolated static 以便从非 actor 上下文安全调用。
    ...
    private func writeLaunchAgentPlist() -> Bool {
        do {
            ...
            log.info("LaunchAgent plist 已写入: \(self.plistPath)")
            return true
        } catch {
            log.error("LaunchAgent plist 写入失败: \(error)")
    ...
    /// 读取已安装 plist 中的 ProgramArguments[0]，即 agent binary 路径。
    ...
    /// 已安装 plist 的 binary 路径与当前 App 内 binary 路径不一致时需要重写。
    ...
    func install() {
        ...
        // 1. 先 unload 旧 job（清理 socket），再写入 plist，确保新 binary 路径生效。
        // 若旧 job 不存在，unload 安静失败无副作用。
        unloadAgentLaunchJobRemovingSocket()
    ...
    func start() {
        ...
        isAgentOperationInProgress = true
        // plist 里的 binary 路径若与当前 App 不一致（重装 App 后常见），先卸载旧 job 并重写 plist
