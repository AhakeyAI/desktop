...
Codex 拆分并落任务卡
  -> 唯一 owner 在指定基线上执行
  -> owner 自验并提交交付包
  -> Codex 只读审查：证据裁决 + 归因诊断 + 优化方案
  -> 接受 / 机械返工 / 诊断设计检查点 / 阻塞 / 重新切片
...
Codex 的验收必须逐条映射任务卡完成定义，输出可定位到文件/测试/日志的 finding。没有证据的“看起来完成”不能更新 WBS 状态。每次验收（包括 accepted）还必须按 [`docs/collab/CODEX-REVIEW-DIAGNOSIS-SOP.md`](collab/CODEX-REVIEW-DIAGNOSIS-SOP.md) 输出归因诊断、优化方案和下一轮可执行边界；只列问题、不解释成因和收敛路径的验收记录无效。

### 6.1 Codex 验收诊断门

1. **每轮都诊断**：Codex 必须区分直接触发点、系统性根因、漏检原因和影响面；accepted 也要说明哪些不变量已由证据闭合、剩余风险如何处理，不能只写“0 finding”。
2. **先保留已通过面**：返工单必须明确“保留/冻结”与“仅开放”范围，禁止执行方重做已验收部分或顺手扩范围。
3. **一次孤立缺陷可机械返工**：只有当表示法、接口和测试 oracle 仍正确，且修复不改变公共状态模型时，才允许直接开 `R1/R2...`。
4. **同一语义轴连续两轮失败强制升级**：若连续两轮 finding 属于同一不变量、同一接口/状态表示、同一测试 oracle，或新反例只是前一补丁的相邻组合，则停止继续追加分支，当前实现卡保持 `review/blocked`，Codex 建立 docs-only 诊断设计检查点。不得开第三轮补丁卡绕过。
5. **以下情形首轮即可升级**：公共接口/数据布局/恢复状态机需要变化；生产路径与测试 seam 不同源；环境失败被负向门禁误判为成功；三个以上模块共同承担同一不变量；finding 证明原任务分解或完成定义本身错误。
6. **设计先于实现**：诊断设计必须冻结问题模型、不变量、状态/失败表、深模块接口、替代方案、迁移范围、永久红测试/突变体、路径白名单和完成定义。Codex 对终版负责；可让执行方提供事实或草稿，但未被 Codex 明确 accepted 前不得恢复产品实现。
7. **WBS 重新切片**：设计确认后，Codex 将后续工作拆成“表示/核心 → 生产接线 → 集成/迁移 → HIL/发布”中实际需要的最小切片，并标明依赖。不得把尚未证明的核心与真机写入混成一次返工。
8. **反馈落盘**：验收结论必须追加到任务卡；涉及架构或跨卡优化时另存 `docs/collab/reports/<ID>-REVIEW-DIAGNOSIS.md`，并在 board 链接。完整模板见上述 SOP。
