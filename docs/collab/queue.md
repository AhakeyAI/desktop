...
| 7 | `WBS-1-UNIFIED-FIRMWARE` | Zcode | 1.1-1.7 | ready / 1.5 slice 2 implementation B1R2 | B1R1 hash 机制已验有效；最后收口自动 mutation negative/共用 pin、header 自包含、legacy 全 ABI、clamped completion-once；B2 冻结 |
...
| 15F2 | `HIL-RELEASE-0.2-INSTALLER-RECOVERY-REWORK` | Cursor；Codex 验收 | 5.9A-R8 / 6.0A | ready / R3 | 收口单一 identity/HIL plist、Outcome terminal snapshot+mutation receipt、App integrity/原树终态；安装重跑仍禁止 |
