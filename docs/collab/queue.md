...
规则：每个写入域默认仅一张卡可处于 `ready/active/review`。当前 Cursor 唯一 active 卡是 `HIL-RELEASE-0.2.1` Gate-1（已提交提审）；Zcode 继续只写独立固件仓 WBS 1.5 B3R2。刷机、reboot/logout、push 和量产切换仍未开放。DMG verifier 失败路径卫生独立排在 Gate-1 之后，不阻断本 SHA 安装，但阻断下一次候选重冻结。
...
| 7 | `WBS-1-UNIFIED-FIRMWARE` | Zcode | 1.1-1.7 | ready / 1.5 slice 2 implementation B3R2 | `27a8d90` T8/T25 收口；只补 T9 payload→RAM 真组合；B4 冻结；不刷机 |
