...
规则：每个写入域默认仅一张卡可处于 `ready/active/review`。当前 Cursor 唯一 ready 卡是 `RELEASE-DMG-VERIFIER-CLEANUP`；Zcode 继续只写独立固件仓 WBS 1.5 B4（0x80/0x81）。v0.2.1 build 360 已安装且主链通过；V021 产品 accepted，须先修 DMG verifier 失败路径 detach，再冻结 build >360 重跑 Gate-1。刷机、reboot/logout、push 和量产切换仍未开放。
