...
规则：每个写入域默认仅一张卡可处于 `ready/active/review`。当前 Cursor 唯一 active 卡是 `HIL-RELEASE-0.2.1` Gate-1；Zcode 继续只写独立固件仓 WBS 1.5 B3R1。Gate-0 候选已 accepted，用户已预授权在当前 Mac 覆盖安装真机测试；刷机、reboot/logout、push 和量产切换仍未开放。DMG verifier 失败路径卫生独立排在 Gate-1 之后，不阻断本 SHA 安装，但阻断下一次候选重冻结。
...
| 7 | `WBS-1-UNIFIED-FIRMWARE` | Zcode | 1.1-1.7 | ready / 1.5 slice 2 implementation B3R1 | `b4f94d9` 单入口/六格表保留；只补 T8/T9/T25 生产组合与真 oracle；B4 冻结；不刷机 |
...
| 15I | `HIL-RELEASE-0.2.1` | Cursor；Codex 验收 | v0.2.1 增量发布 | active / Gate-1 | build 360 / SHA `9f109421…10b46c3` Gate-0 accepted；按预授权安装当前 Mac 并做 Runtime/XPC/BLE/Hook 回归 |
| 15J | `RELEASE-DMG-VERIFIER-CLEANUP` | Cursor；Codex 验收 | release tooling hygiene | queued | 15I Gate-1 收口后修复失败路径 detach；下一次候选重冻结前必须 accepted |
