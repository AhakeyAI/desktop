...
规则：每个写入域默认仅一张卡可处于 `ready/active/review`。当前 Cursor 唯一 active 卡是 `HIL-RELEASE-0.2.1` Gate-1；Zcode 继续只写独立固件仓 WBS 1.5 B3。Gate-0 候选已 accepted，用户已预授权在当前 Mac 覆盖安装真机测试；刷机、reboot/logout、push 和量产切换仍未开放。DMG verifier 失败路径卫生独立排在 Gate-1 之后，不阻断本 SHA 安装，但阻断下一次候选重冻结。
