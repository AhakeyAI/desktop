...
规则：每个写入域默认仅一张卡可处于 `ready/active/review`。当前 Cursor 已停手，E-1R1 处于 Codex review；Zcode 只写独立固件仓 WBS 1.5 R16。v0.2 后续卡现在只登记为 draft，不能抢占当前 owner。Codex 接受当前卡后，在同一轮检查同通道下一张依赖；遇到 `USER-GATE` 时暂停并向用户确认。
...
| 7 | `WBS-1-UNIFIED-FIRMWARE` | Zcode | 1.1-1.7 | active / 1.5 slice 1 R16 | R15 门禁假绿；先闭合 journal/legacy 格式歧义与真实故障注入，切片 2 阻塞 |
...
| 15D | `STUDIO-OLED-ENCODE-AND-PARTIAL-APPLY` | Cursor | HIL C1 编辑/受理缺口 | review / E-1R1 | `4cc56a7` 已提审；Codex 独立复验后裁决 |
| 15E | `RELEASE-0.2-COMPATIBILITY` | Cursor | v0.2 功能策略 | draft | E-1R1 review 已关闭（accepted 或冻结到 v0.3）；客户端写入域空闲 |
...
队列不是一般并行许可。Cursor 已提交 E-1R1 并停手，当前由 Codex review；Zcode 仅执行独立固件仓 WBS 1.5 R16。v0.2 三张后续卡均为 draft：E-1R1 review 关闭后先做兼容策略，再做 5.9A，最后经用户门禁跑 v0.2 HIL；若 E-1R1 仍需大返工则冻结到 v0.3，不阻塞兼容卡。HIL-CONFIG 继续 blocked，待 WBS 1.5 + 刷机门禁后归 v0.3；WBS 2/3/5.8/5A/6 不能反向阻塞 v0.2。刷机、安装、远端 push 和量产切换仍需 USER-GATE。
