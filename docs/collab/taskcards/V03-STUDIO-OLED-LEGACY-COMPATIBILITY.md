...
### [2026-09-07 17:41] Codex 手工验收 C4R13；产品 helper 隔离通过，提交范围与 mutation barrier 未闭；退 C4R14

- Agent Relay 保持暂停；固定审查 `ebb30e2d3730c81d834f2e6464e443999ecf8865...052b8330cdddb8f9ddb56630437401738e4616fd`，未调用 Relay。Codex 独立定向 **369/369**，range `git diff --check` 通过；提交声明其余门禁为连续 5 轮、全量 **1032 / 2 skipped / 0 failures**、App + Agent Release 通过。
- 已通过并冻结：生产 Shared 不再识别 C4R12 env/执行 exit；helper 位于独立 Tests executable 且 App/Agent 不依赖；parent/child Store 都在 MUTATE 前 READY；放锁后 child 真 accept 完成；`sharesLock` internal + closed-aware。
- **P1 — 提交范围污染。** `052b833` 将 `board.md` +983/-8、本任务卡 +724/-9 一并提交，包含大量 C4R13 之前的 Codex 裁决、Zcode 固件/WBS 记录和 old status，不是获准的 Cursor 当轮 append。必须使 `ebb30e2...<new>` 净差异恢复为 C4R13/R14 白名单；不得丢失真实协作记录，可在产品提交后重新保留为工作区手工 diff。
- **P2 — READY→MUTATE 还没证明 child 已进入写竞争。** child 收到 MUTATE 后不发任何 barrier，parent 把 2.5s 任意沉默当 BLOCKED。child 若只是未被调度，放锁后才运行 accept，也会假绿。必须增 `MUTATING`/attempt barrier，parent 先收到该信号再检查持锁期间无 completion。
- **P2 — flock oracle 将任意错误当成 BLOCKED。** helper 未检查 errno，`EBADF/EINTR/EIO` 等都会 exit 0 并输出 BLOCKED。仅 `EWOULDBLOCK/EAGAIN` 可视为互斥成功，其他错误与 unlock 失败必须红。
- C4R14 仅允许：（1）用后续矫正提交使 `ebb30e2...<new>` 净范围只含 C4R13/R14 产品/helper/测试/evidence 与 Cursor 当轮 append，恢复的 Codex/Zcode/manual docs 保留为工作区 diff；（2）child 在读 MUTATE 后、进 `accept` fence 前发 MUTATING，parent 以 barrier 而非沉默裁定；（3）flock helper 严格 errno/unlock 分类；（4）连续至少 5 轮 369/369。
- 白名单限 Package/test helper/test 协议、为净化提交必要的 board/taskcard 矫正、单一 C4R14 evidence 和 Cursor append。不改 Shared Store 语义、C2 assembler、C3 WAL/CAS/事务转移/BLE executor、Studio UI、Hook/安装器/固件。C5/HIL/打包/签名/安装/刷机/push 继续关闭。
- 需要回复：是（@Cursor ACK 后仅执行 C4R14）

### [2026-09-07 18:05] Cursor ACK：开始 C4R14 clean review range / MUTATING barrier / strict flock oracle

ACK 用户转发的 Codex C4R13 未通过与 C4R14 开放。C1–C3 accepted @ `c6e0762`；C4 交互骨架保留 @ `62afcaf`；C4R1–C4R12 已通过项冻结。产品基线 `1ed560b` / 已安装 `0.2.1 (362)` 不受影响。未改 queue/status。未伪造 Relay review_decision。仅执行 C4R14：矫正 `052b833` 夹带的既有 Codex/Zcode/firmware/old-status，使 `ebb30e2...<new>` 只含 C4R13/R14 产品/helper/测试/evidence 与 Cursor 当轮 append；真实协作记录保留为工作区 diff。child 读 MUTATE 后、进 `accept` fence 前发 `MUTATING`；parent 先收 barrier 再断言持锁期间无 completion。flock helper 仅 `EWOULDBLOCK/EAGAIN` 为 BLOCKED，其他 errno 与 unlock 失败非零。不改 Shared Store 语义、C2 assembler、C3 WAL/CAS/事务转移/BLE executor。C5/HIL/打包/签名/安装/刷机/push 仍关闭。完成后停手提审，不自动进 C5。

### [2026-09-07 18:55] Cursor：C4R14 完成，停手提审

ACK 用户转发的 Codex C4R13 未通过与 C4R14 开放。C1–C3 accepted @ `c6e0762`；C4 交互骨架保留 @ `62afcaf`；C4R1–C4R12 已通过项冻结。C4R13 已通过项冻结。产品提交 `30cfeb8`。产品基线 `1ed560b` / 已安装 `0.2.1 (362)` 不受影响。`ebb30e2...<new>` 净范围只含 C4R13/R14 产品/helper/测试/evidence 与 Cursor append；既有 Codex/Zcode/firmware 记录保留为工作区 diff。READY→MUTATE 增加 `MUTATING` barrier。flock 仅 would-block 为 BLOCKED，unlock 必须成功。定向连续 5 轮 369/369，全量 1032/2 skipped/0，App+Agent Release OK。证据 `docs/collab/evidence/V03-STUDIO-OLED-20260902/46-c4r14-clean-range-mutating-flock-oracle.md`。未改 queue/status。未伪造 Relay review_decision。C5/HIL/打包/签名/安装/刷机/push 仍关闭。停手提审，不自动进 C5。
需要回复：是（@Codex 验收 C4R14）

### [2026-09-07 19:04] Codex 手工验收 C4R14：双轴零 finding，C4 accepted @ `30cfeb8`

- Agent Relay 保持暂停；固定审查 `ebb30e2d3730c81d834f2e6464e443999ecf8865...30cfeb889bfea299a41224222f8a5b400df2de33`，未调用 Relay。Standards **0 findings**；Spec **0 findings**。
- 净范围成立：8 个授权文件，board +18、本卡 +17 均为 Cursor C4R13/R14 当轮 append；`052b833` 夹带的既有 Codex/Zcode/firmware/status 已从产品净差异移除，真实手工记录保留为工作区 diff。
- 生产 Shared 无 process-probe/env/exit 路径；helper 位于 Tests 独立 executable，App/Agent 不依赖。READY→MUTATE→MUTATING→GOT_LOCK 证明两个已打开 Store 的真 `accept` 在 parent fence 下阻塞、放锁后完成。flock 仅 `EWOULDBLOCK/EAGAIN` 是 BLOCKED，open/其他 errno/unlock 失败均非零。
- Codex 独立复跑定向 **369/369**，产品 range `git diff --check` 通过。提交声明其余门禁为定向连续 5 轮、全量 **1032 / 2 skipped / 0 failures**、App + Agent Release 通过。
- C1–C4 客户端切片验收完成。**C5 未开放**，须用户另行明确授权；HIL/打包/签名/安装/刷机/push 继续关闭。
- 需要回复：是（等待用户决定是否开放 C5）

### [2026-09-07 19:04] Codex：用户已授权开启 C5

- C4 accepted @ `30cfeb8`；C5 转由 `HIL-V03-STUDIO-OLED-COMPATIBILITY` 执行，状态为 `ready / C5 preflight`。
- 本次授权允许冻结候选、复验构建与正式 UI/HIL 矩阵的非破坏前置工作；签名/打包/安装、断电、每次固件切换/刷入、EEPROM 擦除仍按 HIL 卡要求逐次请求用户授权。
- 需要回复：否（转 15L 执行）

### [2026-09-07 19:12] Cursor ACK：开始 C5 非破坏 preflight

ACK 用户 19:07 确认并开启 C5。C1–C4 accepted @ `30cfeb8`。执行转 15L `HIL-V03-STUDIO-OLED-COMPATIBILITY`。Agent Relay 继续暂停。未改 queue/status。未伪造 Relay review_decision。仅执行非破坏 preflight：冻结 `30cfeb8`；复验 App/Agent Release、版本/签名计划、现网 XPC/唯一 owner；准备 PNG/JPEG/GIF、>2 MiB/120 帧、超限/损坏夹具与取证模板；只读设备 identity/固件/已有图片/EEPROM 初态与三族 SHA/HEX。正式 Studio UI + 正式 Runtime。不 overlay `/Applications`、不 kickstart、不刷机、不擦 EEPROM、不 push、不签名/打包/安装。完成后停手提交下一精确 USER-GATE。

### [2026-09-07 19:25] Cursor：C5 非破坏 preflight 完成，停手

ACK 用户 19:07 开启 C5。C1–C4 accepted @ `30cfeb8`。15L preflight 完成：现网唯一 owner + XPC ok，未 kickstart；三族 HEX 已登记；正式 UI 仍被 `.v0_2` 功能策略挡住。证据 `docs/collab/evidence/HIL-V03-STUDIO-OLED-20260907/00-c5-preflight.md`。未改 queue/status。未 overlay/签名/安装/刷机/push。下一门 **USER-GATE-C5-POLICY**。
需要回复：是（@Codex 验收 15L preflight）
