...
public class BleManager {
    ...
    public interface BleCallback {
        ...
        /**
         * 按键事件回调
         * @param hidCode HID按键码
         * @param isKeyDown true=按下, false=释放
         */
        void onKeyEvent(int hidCode, boolean isKeyDown);
    }
    ...
    // 按键事件命令码
    private static final byte CMD_KEY_EVENT = (byte) 0x93;
    ...
    private void onBleNotify(byte[] data) {
        ...
        if (data.length >= 7) {
            ...
            if (cmd == CMD_KEY_EVENT) {
                // 解析按键事件: [0xAA][0xBB][0x93][hidCode][isKeyDown][0xCC][0xDD]
                int hidCode = data[3] & 0xFF;
                boolean isKeyDown = (data[4] & 0xFF) == 1;
                logger.info("收到按键事件: HID=0x{}, down={}", Integer.toHexString(hidCode), isKeyDown);
                callback.onKeyEvent(hidCode, isKeyDown);
                return;
            }
        }
