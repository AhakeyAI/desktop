...
final class AhaKeyAgentPageExecutionTests: XCTestCase {
    ...
    func testXPCAbandonAfterSixtySecondDisconnectReleasesHead() {
        runTest { [self] in
            ...
            let pausedHead = await waitUntil(agent, head.operationID, states: [.paused, .resumablePartial])
            XCTAssertNotNil(pausedHead)
            ...
            let settledHead = await waitUntil(agent, head.operationID, states: [.paused, .resumablePartial])
            XCTAssertNotNil(settledHead)
    ...
    func testAbandonEligibilityFlipsAtSixtySecondsWithoutOtherEvents() {
        runTest { [self] in
            ...
            let pausedAfterQueued = await waitUntil(
                agent, head.operationID, states: [.paused, .resumablePartial]
            )
            XCTAssertNotNil(pausedAfterQueued)
            await agent.noteProductionDisconnectForTesting()
            await agent.simulateDeviceForTesting(
                simulatedDevice(protocolState: .disconnected, bluetoothConnected: false)
            )
            let pausedAfterDisconnect = await waitUntil(
                agent, head.operationID, states: [.paused, .resumablePartial]
            )
            XCTAssertNotNil(pausedAfterDisconnect)
    ...
    func testFreshAgentRearmsExistingEpochAtFiftyNineThenFlipsAtSixty() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            let storeDir = try XCTUnwrap(agent.executionTestHooks?.storeDirectory)
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let head = try statusPackage(device: "TEST-DEVICE", seed: "c4r3-reopen-head")
            let queued = try statusPackage(device: "TEST-DEVICE", seed: "c4r3-reopen-queued")
            guard case .operationAccepted = try await client.exchange(.apply(head)) else {
                return XCTFail("head apply")
            }
            await waitUntil(agent, head.operationID, states: [.paused, .resumablePartial])
            ...
            now = now.addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration - 1)
            let reopened = try await makePreparedAgent(storeDirectory: storeDir)
            reopened.runtimeEventsLongPollInterval = 0.12
            await reopened.simulateDeviceForTesting(
                simulatedDevice(protocolState: .disconnected, bluetoothConnected: false)
            )
            var reopenHooks = reopened.executionTestHooks
            ...
            let reopenClient = EndpointClient(agent: reopened)
            try await reopenClient.handshake()
            guard case .snapshot(let at59) = try await reopened.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("59s reopen snapshot")
            }
            XCTAssertEqual(
                at59.operations.first { $0.id == head.operationID }?.abandonEligibility?.eligible,
                false
            )
            XCTAssertNotEqual(
                at59.operations.first { $0.id == queued.operationID }?.abandonEligibility?.eligible,
                true
            )
            let cursor = at59.latestEventSequence
            now = now.addingTimeInterval(1)
            var due = reopened.executionTestHooks
            due?.wallClock = { now }
            reopened.executionTestHooks = due
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let at60Replay) = try await reopened.handleRuntimeXPCRequest(
                .events(after: cursor)
            ),
            case .events(let at60) = at60Replay else {
                return XCTFail("60s reopen events")
            }
            let eligibleHead = at60.compactMap { event -> Bool? in
                guard case .operationChanged(let summary) = event.payload,
                      summary.id == head.operationID else { return nil }
            ...
            XCTAssertFalse(queuedEligible)
            guard case .snapshot(let at60Snapshot) = try await reopened.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("60s reopen snapshot")
            }
            XCTAssertEqual(
                at60Snapshot.operations.first { $0.id == head.operationID }?.abandonEligibility?.eligible,
                true
            )
            XCTAssertNotEqual(
                at60Snapshot.operations.first { $0.id == queued.operationID }?.abandonEligibility?.eligible,
                true
            )
    ...
    func testTwoDeviceHeadsKeepIndependentDeadlinesAtSixtyAndNinety() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            let packageA = try statusPackage(device: "TEST-DEVICE", seed: "c4r4-device-a")
            guard case .operationAccepted = try await client.exchange(.apply(packageA)) else {
                return XCTFail("A apply")
            }
            let pausedA = await waitUntil(agent, packageA.operationID, states: [.paused, .resumablePartial])
            XCTAssertNotNil(pausedA)
            await agent.noteProductionDisconnectForTesting()
            await agent.simulateDeviceForTesting(
                simulatedDevice(protocolState: .disconnected, bluetoothConnected: false)
            )

            let packageB = try statusPackage(device: "DEVICE-B", seed: "c4r4-device-b")
            let lease = try XCTUnwrap(agent.cachedWriterLeaseForTesting())
            let store = try await agent.runtimeStoreForTesting()
            _ = try await store.accept(packageB, resourceFiles: [:])
            guard let acceptedB = try await store.transaction(packageB.operationID) else {
                return XCTFail("B WAL row")
            }
            try await store.updateOperation(
                try AhaKeyRuntimeOperationSummary(
                    id: acceptedB.operationID,
            ...
            now = Date(timeIntervalSince1970: 1_700_000_000)
                .addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration - 1)
            var at59Hooks = agent.executionTestHooks
            at59Hooks?.wallClock = { now }
            agent.executionTestHooks = at59Hooks
            guard case .snapshot(let at59) = try await agent.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("59s snapshot")
            }
            XCTAssertEqual(
                at59.operations.first { $0.id == packageA.operationID }?.abandonEligibility?.eligible,
                false
            )
            XCTAssertEqual(
                at59.operations.first { $0.id == packageB.operationID }?.abandonEligibility?.eligible,
                false
            )
            let cursor = at59.latestEventSequence

            now = Date(timeIntervalSince1970: 1_700_000_000)
                .addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration)
            var at60Hooks = agent.executionTestHooks
            at60Hooks?.wallClock = { now }
            agent.executionTestHooks = at60Hooks
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let at60Replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: cursor)
            ),
            ...
            let aEligibleAt60 = at60.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == packageA.operationID && summary.abandonEligibility?.eligible == true
            }
            let bEligibleAt60 = at60.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == packageB.operationID && summary.abandonEligibility?.eligible == true
            }
            XCTAssertTrue(aEligibleAt60)
            XCTAssertFalse(bEligibleAt60)
            guard case .snapshot(let at60Snapshot) = try await agent.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("60s snapshot")
            }
            XCTAssertEqual(
                at60Snapshot.operations.first { $0.id == packageA.operationID }?.abandonEligibility?.eligible,
                true
            )
            XCTAssertEqual(
                at60Snapshot.operations.first { $0.id == packageB.operationID }?.abandonEligibility?.eligible,
                false
            )

            now = Date(timeIntervalSince1970: 1_700_000_000)
                .addingTimeInterval(30 + AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration)
            var at90Hooks = agent.executionTestHooks
            at90Hooks?.wallClock = { now }
            agent.executionTestHooks = at90Hooks
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let at90Replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: at60Snapshot.latestEventSequence)
            ),
            case .events(let at90) = at90Replay else {
                return XCTFail("90s events")
            }
            let bEligibleAt90 = at90.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == packageB.operationID && summary.abandonEligibility?.eligible == true
            }
            XCTAssertTrue(bEligibleAt90)
            await agent.closeRuntimeStoreForTesting()
        }
    }

    func testAbandonDeadlineRetriesAfterFailedPublication() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r5-retry")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
    ...
    func testOldAbandonWakeDoesNotConsumeReplacementEpoch() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let t0 = now
            let agent = try await makeReadyAgent()
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r5-replace")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            hooks = agent.executionTestHooks
            hooks?.abandonEligibilityRefreshGate = {
                do {
                    let store = try await agent.runtimeStoreForTesting()
            ...
            XCTAssertEqual(
                at59.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                false
            )
            let cursor = at59.latestEventSequence
            now = t0.addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration)
            var at60 = agent.executionTestHooks
            at60?.wallClock = { now }
            agent.executionTestHooks = at60
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let at60Replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: cursor)
            ),
            case .events(let at60Events) = at60Replay else {
                return XCTFail("60s events")
            }
            let eligibleAt60 = at60Events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
            }
            XCTAssertFalse(eligibleAt60)
            ...
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let at90Replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: at59.latestEventSequence)
            ),
            case .events(let at90Events) = at90Replay else {
                return XCTFail("90s events")
            }
            let eligibleAt90 = at90Events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
            }
            XCTAssertTrue(eligibleAt90)
            await agent.closeRuntimeStoreForTesting()
        }
    }

    func testFreshAgentConnectedBStillArmsDisconnectedA() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let packageA = try statusPackage(device: "TEST-DEVICE", seed: "c4r5-device-a")
            guard case .operationAccepted = try await client.exchange(.apply(packageA)) else {
                return XCTFail("A apply")
            }
            let pausedA = await waitUntil(agent, packageA.operationID, states: [.paused, .resumablePartial])
            ...
            let aEligible = at60.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == packageA.operationID && summary.abandonEligibility?.eligible == true
            }
    ...
    func testProofBoundaryReconnectDoesNotPublishStaleEligible() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r7-boundary-reconnect")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            let cursor = at59.latestEventSequence
            let reconnected = simulatedDevice(displayName: "n-plus-1", transportGeneration: 1)
            var boundary = agent.executionTestHooks
            boundary?.abandonEligibilityPublishBoundaryGate = {
                await agent.simulateDeviceForTesting(reconnected)
                try? await agent.noteProductionReconnectForTesting()
            }
            agent.executionTestHooks = boundary
            now = now.addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration)
            var due = agent.executionTestHooks
            due?.wallClock = { now }
            agent.executionTestHooks = due
            ...
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(.events(after: cursor)),
                  case .events(let events) = replay else {
                return XCTFail("boundary events")
            }
            let eligible = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
    ...
    func testProofBoundaryHeadChangeDoesNotPublishStaleEligible() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r7-boundary-head")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
                ...
                do {
                    ...
                    try await store.updateOperation(
                        try AhaKeyRuntimeOperationSummary(
                            ...
                            totalSteps: max(record.totalSteps, 1),
                            durableOrdering: record.durableOrdering ?? .live(queueOrder: 1)
                        )
                    )
                } catch {
                    XCTFail("head change: \(error)")
                }
            }
            agent.executionTestHooks = boundary
            now = now.addingTimeInterval(AhaKeyRuntimeAbandonPolicy.requiredDisconnectedDuration)
            ...
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(.events(after: cursor)),
                  case .events(let events) = replay else {
                return XCTFail("head-change events")
            }
            let eligible = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
    ...
    func testPostProofTerminalCommitDoesNotPublishStaleEligible() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            agent.runtimeEventsLongPollInterval = 0.12
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r8-post-proof-terminal")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            postProof?.abandonEligibilityPostProofGate = {
                do {
                    ...
                    XCTFail("post-proof terminal: \(error)")
                }
            }
            ...
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(.events(after: cursor)),
                  case .events(let events) = replay else {
                return XCTFail("post-proof events")
            }
            let eligible = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
    ...
    func testPostProofReconnectDoesNotPublishStaleEligible() {
        runTest { [self] in
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r8-post-proof-reconnect")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(.events(after: cursor)),
                  case .events(let events) = replay else {
                return XCTFail("post-proof reconnect events")
            }
            let eligible = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
    ...
    func testSameStartedAtDifferentEpochIdentityEmitsNewSequence() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let t0 = now
            let agent = try await makeReadyAgent()
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r7-same-started")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            agent.executionTestHooks = due
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .snapshot(let first) = try await agent.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("first snapshot")
            }
            XCTAssertEqual(
                first.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                true
            )
            let firstSequence = first.latestEventSequence
            let lease = try XCTUnwrap(agent.cachedWriterLeaseForTesting())
            let deviceID = try AhaKeyRuntimeDeviceID("TEST-DEVICE")
            let replacement = AhaKeyRuntimeDisconnectEpoch(
                identity: AhaKeyRuntimeConnectionIdentity(
            ...
            _ = try await agent.handleRuntimeXPCRequest(.snapshot)
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: firstSequence)
            ),
            case .events(let events) = replay else {
                return XCTFail("replacement events")
            }
            let replacementEvents = events.compactMap { event -> AhaKeyRuntimeEventSequence? in
                guard case .operationChanged(let summary) = event.payload else { return nil }
                guard summary.id == package.operationID,
                      summary.abandonEligibility?.eligible == true,
                      summary.abandonEligibility?.epochIdentity?.sessionGeneration == .init(2) else {
                    return nil
                }
                return event.sequence
            }
            XCTAssertFalse(replacementEvents.isEmpty)
            XCTAssertTrue(replacementEvents.allSatisfy { $0 > firstSequence })
            await agent.closeRuntimeStoreForTesting()
        }
    }

    func testDeviceAExhaustedRetryDoesNotBlockDeviceBDeadline() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let t0 = now
            let agent = try await makeReadyAgent()
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            hooks?.abandonEligibilityRefreshFailuresRemaining = 1_000
            hooks?.abandonEligibilityRefreshFailureDeviceID = try AhaKeyRuntimeDeviceID("TEST-DEVICE")
            agent.executionTestHooks = hooks

            let packageA = try statusPackage(device: "TEST-DEVICE", seed: "c4r7-fail-a")
            guard case .operationAccepted = try await client.exchange(.apply(packageA)) else {
                return XCTFail("A apply")
            }
            let pausedA = await waitUntil(agent, packageA.operationID, states: [.paused, .resumablePartial])
            ...
            let packageB = try statusPackage(device: "DEVICE-B", seed: "c4r7-ok-b")
            let lease = try XCTUnwrap(agent.cachedWriterLeaseForTesting())
            let store = try await agent.runtimeStoreForTesting()
            _ = try await store.accept(packageB, resourceFiles: [:])
            guard let acceptedB = try await store.transaction(packageB.operationID) else {
            ...
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let replay) = try await agent.handleRuntimeXPCRequest(
                .events(after: AhaKeyRuntimeEventSequence(0))
            ),
            case .events(let events) = replay else {
                return XCTFail("90s events")
            }
            let bEligible = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == packageB.operationID && summary.abandonEligibility?.eligible == true
            }
            XCTAssertTrue(bEligible)
            await agent.closeRuntimeStoreForTesting()
        }
    }

    func testStaleRetrySleeperDoesNotWakeReplacementToken() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let t0 = now
            let agent = try await makeReadyAgent()
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            hooks?.abandonEligibilityRefreshFailuresRemaining = 1_000
            hooks?.abandonEligibilityRefreshFailureDeviceID = deviceID
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r8-stale-sleeper")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            let paused = await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            agent.executionTestHooks = due
            try? await Task.sleep(nanoseconds: 80_000_000)
            let retryCount = await agent.abandonRetryTaskCountForTesting()
            XCTAssertGreaterThan(retryCount, 0)
            let pending = await agent.pendingAbandonTokenForTesting(deviceID)
            let oldToken = try XCTUnwrap(pending)
            let lease = try XCTUnwrap(agent.cachedWriterLeaseForTesting())
            let replacement = AhaKeyRuntimeDisconnectEpoch(
                identity: AhaKeyRuntimeConnectionIdentity(
                    deviceID: deviceID,
                ...
                startedAt: oldToken.epoch.startedAt
            )
            let store = try await agent.runtimeStoreForTesting()
            try await store.mintDisconnectEpoch(package.operationID, epoch: replacement)
            await agent.replacePendingAbandonDeadlineForTesting(
            ...
            let baseline = agent.executionTestHooks?.abandonEligibilityRefreshAttemptCount ?? 0
            try? await Task.sleep(nanoseconds: 50_000_000)
            let later = agent.executionTestHooks?.abandonEligibilityRefreshAttemptCount ?? 0
            XCTAssertEqual(later, baseline)
            var recovered = agent.executionTestHooks
            recovered?.abandonEligibilityRefreshFailuresRemaining = 0
            recovered?.abandonEligibilityRefreshFailureDeviceID = nil
            agent.executionTestHooks = recovered
            await agent.replacePendingAbandonDeadlineForTesting(
                AhaKeyAbandonDeadlineToken(
                    operationID: package.operationID,
            ...
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .snapshot(let snap) = try await agent.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("recovered snapshot")
            }
            XCTAssertEqual(
                snap.operations.first { $0.id == package.operationID }?.abandonEligibility?.epochIdentity?.sessionGeneration,
                .init(4)
            )
            XCTAssertEqual(
                snap.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                true
            )
            await agent.closeRuntimeStoreForTesting()
            let remaining = await agent.abandonRetryTaskCountForTesting()
            XCTAssertEqual(remaining, 0)
        }
    }

    func testFreshAgentAlreadyDueEpochPublishesWithoutNewMint() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            let storeDir = try XCTUnwrap(agent.executionTestHooks?.storeDirectory)
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r3-due-reopen")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            let reopenClient = EndpointClient(agent: reopened)
            try await reopenClient.handshake()
            guard case .snapshot(let snapshot) = try await reopened.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("due snapshot")
            }
            XCTAssertEqual(
                snapshot.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                true
            )
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .eventReplay(let replay) = try await reopened.handleRuntimeXPCRequest(
                .events(after: snapshot.latestEventSequence)
            ),
            case .events(let events) = replay else {
                return XCTFail("due events")
            }
            let published = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
            }
    ...
    func testFreshAgentConnectedDoesNotArmExistingEpoch() {
        runTest { [self] in
            var now = Date(timeIntervalSince1970: 1_700_000_000)
            let agent = try await makeReadyAgent()
            let storeDir = try XCTUnwrap(agent.executionTestHooks?.storeDirectory)
            ...
            hooks?.stepExecutor = { _ in .retryableFailure }
            agent.executionTestHooks = hooks
            let package = try statusPackage(device: "TEST-DEVICE", seed: "c4r3-connected-reopen")
            guard case .operationAccepted = try await client.exchange(.apply(package)) else {
                return XCTFail("apply")
            }
            await waitUntil(agent, package.operationID, states: [.paused, .resumablePartial])
            ...
            try await reopenClient.handshake()
            guard case .snapshot(let at59) = try await reopened.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("connected snapshot")
            }
            XCTAssertNotEqual(
                at59.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                true
            ...
            reopened.executionTestHooks = due
            try? await Task.sleep(nanoseconds: 80_000_000)
            guard case .snapshot(let at60) = try await reopened.handleRuntimeXPCRequest(.snapshot) else {
                return XCTFail("connected 60s snapshot")
            }
            XCTAssertNotEqual(
                at60.operations.first { $0.id == package.operationID }?.abandonEligibility?.eligible,
                true
            )
            guard case .eventReplay(let replay) = try await reopened.handleRuntimeXPCRequest(
                .events(after: cursor)
            ...
            case .events(let events) = replay else {
                return XCTFail("connected events")
            }
            let published = events.contains { event in
                guard case .operationChanged(let summary) = event.payload else { return false }
                return summary.id == package.operationID && summary.abandonEligibility?.eligible == true
    ...
    private func makePreparedAgent(
        ...
        if storeDirectory == nil {
            return try await makePreparedAgentOnce(
        ...
        for _ in 0..<30 {
            do {
                ...
                guard text.contains("locked") || text.contains("SQLITE_BUSY") else { throw error }
    ...
    private func makePreparedAgentOnce(
        storeDirectory: URL?,
        userSlotLimit: Int,
        objectSeed: String?
    ) async throws -> AhaKeyAgent {
