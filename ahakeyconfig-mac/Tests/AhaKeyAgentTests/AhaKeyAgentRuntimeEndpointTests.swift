...
final class AhaKeyAgentRuntimeEndpointTests: XCTestCase {
    ...
    /// C5IR8：**唯一**产品树集成门 —— 全 `Sources/**` 直接喂给 `SwiftSourceBoundaryAudit` 深模块。
    ///
    /// 深模块自己负责「注释/字符串感知 + 插值递归 + 声明排除」；本门只冻结两件产品事实：
    /// 1. 全树零 violation（不存在非字面量 callsite / 越界 opcode / authority 回读引用）；
    /// 2. 直接命令帧 callsite 的 opcode multiset 精确等于既有 `0x00`（状态轮询）与 `0x94`（legacy 探测）。
    ///
    /// 旧 C5IR2–C5IR7 的 lexer / 调用解析 / authority 计数已全部删除，不再与深模块双轨并存。
    func testAgentBoundaryAuditGateKeepsProductSourceInventoryFrozen() throws {
        ...
        guard let enumerator = FileManager.default.enumerator(
            ...
            return XCTFail("无法枚举 \(sourcesRoot.path)")
        }
        var files: [SwiftSourceBoundaryAudit.SourceFile] = []
        for case let url as URL in enumerator where url.pathExtension == "swift" {
            let relative = url.path.hasPrefix(packageRoot.path + "/")
                ? String(url.path.dropFirst(packageRoot.path.count + 1))
                : url.path
            files.append(.init(path: relative, bytes: [UInt8](try Data(contentsOf: url))))
        }
        XCTAssertFalse(files.isEmpty, "Sources/** 必须至少有一个 .swift")

        let report = SwiftSourceBoundaryAudit.audit(files)
        guard case let .complete(complete) = report else {
            return XCTFail("产品树审计必须 complete（任一文件 malformed 都是产品级事故），实得 \(report)")
        }
        XCTAssertTrue(
        ...
        XCTAssertEqual(
            complete.calls.map(\.location.path),
            ["Sources/Agent/AhaKeyAgent.swift", "Sources/Agent/AhaKeyAgent.swift"]
        )
        for call in complete.calls {
            XCTAssertTrue(call.location.path.hasPrefix("Sources/"), "路径必须是包内相对路径")
            XCTAssertGreaterThan(call.location.line, 0)
            XCTAssertGreaterThan(call.location.column, 0)
            XCTAssertGreaterThanOrEqual(call.location.utf16Offset, 0)
        }
    }
    ...
    func testStaleQueuedPublicationAfterUnavailableOrShutdownWritesZeroCASAndWAL() {
        runEndpointTest { [self] in
            ...
            for revoke in [Revoke.unavailable, .shutdown] {
                for pictureApply in [false, true] {
                    ...
                    XCTAssertNil(
                        ...
                        "迟到 publication 不得复活 token \(revoke) \(pictureApply)"
                    )
    ...
    func testOrdinaryDeviceChangedAfterRevokeDoesNotRestoreAdmission() {
        runEndpointTest { [self] in
            let agent = makeAgent()
            var hooks = agent.executionTestHooks
            hooks?.skipConfigurationBLEWriteGates = true
            ...
            agent.publishOrdinaryDeviceChangedForTesting()
            try await Task.sleep(nanoseconds: 40_000_000)
            XCTAssertNil(agent.liveAdmissionTokenForTesting())
            ...
            guard case .failure(let code) = ingest else {
                return XCTFail("普通 deviceChanged 不得恢复 admission，实际 \(ingest)")
            }
            XCTAssertEqual(code.rawValue, "unsupported-protocol")
            XCTAssertEqual(casFingerprint(storeDir).names, before.names)
        }
    ...
    func testFreshProofRestoresAdmissionAndStalePublicationDoesNotOverwrite() {
        runEndpointTest { [self] in
            let agent = makeAgent()
            let publication = IngestCASGate()
            var hooks = agent.executionTestHooks
            ...
            var liveHooks = agent.executionTestHooks
            liveHooks?.beforeDeviceChangedPublication = nil
            agent.executionTestHooks = liveHooks
            await agent.simulateDeviceForTesting(simulatedDevice(sessionGeneration: 1))
