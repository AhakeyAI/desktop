...
// MARK: - 配置事务串行执行协调器（WBS-5.7 R2/R4 收口）
//
...
// - R4：每趟先做纯 WAL 取消结算，再按序执行会写设备的包；队首非终态立即停止，
//   后续配置不得越过 paused/resumablePartial。
//
...
public actor AhaKeyConfigurationExecutionCoordinator {
    /// 拉取 WAL 待执行候选包（按受理序；取消请求中的排队事务不得过滤，由结算入口处理）。
    public typealias PendingPackagesProvider = @Sendable () async -> [AhaKeyConfigurationPackage]
    /// 纯 WAL 取消结算（不得发 BLE）。结算任意排队 `cancellationRequested`。
    public typealias SettleQueuedCancellations = @Sendable () async -> Void
    /// 执行单个事务（必须兜住全部错误、不抛出）。返回是否终态。
    ...
    private let settleQueuedCancellations: SettleQueuedCancellations
    ...
    public init(
        ...
        settleQueuedCancellations: @escaping SettleQueuedCancellations,
        executePackage: @escaping ExecutePackage
    ) {
        self.pendingPackagesProvider = pendingPackages
        self.settleQueuedCancellations = settleQueuedCancellations
    ...
    /// worker 主循环：每趟先清 pending 标记，再 WAL 取消结算，再按序执行。
    /// `guard` 判定与 `worker = nil` 之间无挂起点（actor 隔离），kick 不会丢失。
    private func runLoop() async {
        while true {
            kickWhileRunning = false
            await settleQueuedCancellations()
