...
public final class StandbySettingsPane {
    ...
    public VBox create(Stage owner) {
        ...
        Label description = label(text(
            localize("设备无操作达到指定时间后自动待机。支持 USB 或 BLE，保存后会回读校验。"),
        ...
        Button save = new Button(text(localize("保存"), "Save"));
        ...
        var connectionListener = new javafx.beans.value.ChangeListener<Boolean>() {
            @Override public void changed(
                ...
                if (connected) {
                    ...
                    status.setText(text(localize("请先通过 USB 或 BLE 连接设备。"),
                        "Connect the device by USB or BLE first."));
                }
            }
        };
    ...
    private void refresh(
        ...
        if (!deviceStatus.isConnected()) {
            supported.set(false);
            status.setText(text(localize("请先连接设备。"), "Connect the device first."));
            return;
        }
        supported.set(false);
        busy.set(true);
        status.setText(text(localize("正在读取设备设置…"), "Reading device setting…"));
        daemon("standby-read", () -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    supported.set(true);
                    busy.set(false);
                    status.setText(text(localize("已通过 "), "Connected over ")
                ...
                Platform.runLater(() -> {
                    ...
                    busy.set(false);
                    supported.set(false);
                    status.setText(text(localize("读取失败："), "Read failed: ")
        ...
        daemon("standby-save", () -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    busy.set(false);
                    status.setText(text(localize("已保存并验证："), "Saved and verified: ")
                        + minutes(verified));
                });
            } catch (Exception exception) {
                Platform.runLater(() -> {
                    if (!owner.isShowing() || generation.get() != request) return;
                    busy.set(false);
                    String message = text(localize("保存失败："), "Save failed: ")
                        + exception.getMessage();
                    status.setText(message);
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.initOwner(owner);
                    alert.setTitle(text(localize("待机时间设置失败"), "Standby Setting Failed"));
    ...
    private String currentText(Integer value) {
        return value == null
            ? text(localize("当前设置：—"), "Current setting: —")
            : text(localize("当前设置："), "Current setting: ") + minutes(value);
    }

    private String minutes(int value) {
        if (value == 0) return text(localize("永不关机"), "Never power off");
        return text(value + localize(" 分钟"), value + " minutes");
    }
